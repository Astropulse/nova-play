...
export class KnowledgeEvent {
    constructor(game, worldX, worldY) {
        ...
        this.radius = 100 * this.game.worldScale;
        this.innerRadius = 150 * this.game.worldScale; // Trigger radius for boss
        this.bossRadius = 150 * this.game.worldScale; // Current expanding radius
        this.targetBossRadius = 800 * this.game.worldScale;
    ...
    update(dt, player) {
        ...
        if (this.state === KNOWLEDGE_STATE.DEFEATED || this.state === KNOWLEDGE_STATE.FINISHED) {
            ...
            if (this.state === KNOWLEDGE_STATE.DEFEATED && this.explosionCount < 5) {
                ...
                if (this.defeatTimer <= 0) {
                    ...
                    const offX = (Math.random() - 0.5) * 300 * this.game.worldScale;
                    const offY = (Math.random() - 0.5) * 300 * this.game.worldScale;
                    ...
                    for (let i = 0; i < count; i++) {
                        ...
                        const speed = (100 + Math.random() * 300) * this.game.worldScale;
        ...
        if (!this.revealed && dist < 3500 * this.game.worldScale) {
            this.revealed = true;
        }
        ...
        if (isTrackingPlayer) {
            ...
            const displacement = 20 * Math.min(1, dist / (600 * this.game.worldScale));
        ...
        if (this.state === KNOWLEDGE_STATE.BOSS) {
            ...
            if (this.bossRadius < this.targetBossRadius) {
                this.bossRadius = Math.min(this.targetBossRadius, this.bossRadius + 300 * dt * this.game.worldScale);
            }
            ...
            if (dist > this.targetBossRadius + 400 * this.game.worldScale) {
                this._resetBoss();
            }
        }
    }
    ...
    _handleSuction(dt, player) {
        const suctionRadius = 600 * this.game.worldScale;
        const consumeRadius = 15 * this.game.worldScale;
    ...
    _consumeItem(item) {
        ...
        if (up) {
            ...
            if (player) {
                ...
                const speed = 300 * this.game.worldScale;
    ...
    _consumeEnemy(enemy) {
        ...
        if (up) {
            ...
            if (player) {
                const angle = Math.atan2(player.worldY - this.worldY, player.worldX - this.worldX);
                const speed = 400 * this.game.worldScale;
                reward.vx = Math.cos(angle) * speed;
                reward.vy = Math.sin(angle) * speed;
            }
            this.pendingSpawns.push(reward);
    ...
    getSpawnOnDeath() {
        ...
        if (up) {
            ...
            const angle = Math.random() * Math.PI * 2;
            const speed = (30 + Math.random() * 200) * this.game.worldScale;
