...
export class PlayingState {
    ...
    _spawnEvents() {
        ...
        const dist = (20000 + Math.random() * 10000) * this.game.worldScale;
        ...
        const cargoDist = (3000 + Math.random() * 3000) * this.game.worldScale;
        ...
        const f1Dist = (4000 + Math.random() * 2000) * this.game.worldScale;
        ...
        const f2Dist = (6000 + Math.random() * 2000) * this.game.worldScale;
        const f2x = Math.cos(f2Angle) * f2Dist;
        const f2y = Math.sin(f2Angle) * f2Dist;

        // Station 3: Far away
        const f3Angle = Math.random() * Math.PI * 2;
        const f3Dist = (15000 + Math.random() * 5000) * this.game.worldScale;
    ...
    _spawnInitialAsteroids() {
        ...
        for (let i = 0; i < numAsteroids; i++) {
            const angle = Math.random() * Math.PI * 2;
            const dist = (400 + Math.random() * 1600) * this.game.worldScale;
    ...
    update(dt) {
        ...
        if (this.hasRockets) {
            ...
            if (this.player.rocketsTimer <= 0) {
                let target = null;
                let minDist = 1500 * this.game.worldScale;
        ...
        if (this.hasAutoTurret) {
            ...
            if (this.player.autoTurretTimer <= 0) {
                ...
                let target = null;
                let minDist = 800 * this.game.worldScale;
                ...
                if (target) {
                    const noseOffset = 20 * this.game.worldScale;
        ...
        if (this.hasMechanicalClaw) {
            ...
            if (this.player.mechanicalClawTimer <= 0) {
                ...
                for (const en of this.enemies) {
                    ...
                    const edy = en.worldY - this.player.worldY;
                    const edist = Math.sqrt(edx * edx + edy * edy);
                    if (edist < 150 * this.game.worldScale) {
                        en.freeze(3.0);
                        triggered = true;
                    }
                }
        ...
        for (const e of this.enemies) {
            ...
            if (Math.sqrt(dxArr * dxArr + dyArr * dyArr) > 7000) {
                e.alive = false;
            }
        }
    ...
    _spawnExplosion(x, y, damage) {
        ...
        this.explosions.push({
            worldX: x,
            worldY: y,
            radius: 50 * this.game.worldScale,
            ...
            if (!this.game.input.isMouseDown(0)) {
                ...
                // 3. Drop failed: Return to shop or Drop into space
                    ...
                    if (this.draggedItem.originInventory === shopInv) {
                        ...
                        const dropOffset = (Math.random() - 0.5) * 20 * this.game.worldScale;
                        const dropOffset2 = (Math.random() - 0.5) * 20 * this.game.worldScale;
    ...
    spawnDistantShop() {
        ...
        const angle = Math.random() * Math.PI * 2;
        // 15,000 pixels away
        const dist = 15000 * this.game.worldScale;
    ...
    _handlePrimaryWeaponFire() {
        const p = this.player;
        const noseOffset = 36 * this.game.worldScale;
        ...
        const beamLength = 4000 * this.game.worldScale;
    ...
    _spawnExplosion(x, y, damage) {
        ...
        const radius = 64 * this.game.worldScale;
    ...
    _drawExplosions(ctx) {
        ...
        const baseSize = 64 * this.game.worldScale;
